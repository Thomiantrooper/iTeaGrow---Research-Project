...
# These are reference values - adjust based on actual tea industry standards
STANDARD_DENSITY = {
    "BOP": {
        "min": 280,  # grams/liter
        "max": 320,
...
# Pricing Structure (base price per kg in your currency)
BASE_PRICES = {
    "BOP": 1200,
    "BOPF": 1100,
    "Dust": 800,
...
def check_density(grade: str, weight: float, volume: float = 1.0) -> dict:
    """
    Check if the weight-to-volume ratio matches the standard density for the grade.
    
    Args:
        grade: Tea grade (BOP, BOPF, etc.)
        weight: Weight in grams
        volume: Volume in liters (default 1.0)
    
    Returns:
        Dictionary with density check results
    """
    ...
    if within_tolerance:
        if abs(density - optimal_density) / optimal_density <= 0.02:  # Within 2%
            status = "Excellent"
        elif abs(density - optimal_density) / optimal_density <= 0.05:  # Within 5%
            status = "Good"
        else:
            status = "Acceptable"
    else:
...
def calculate_price(grade: str, density_check: dict) -> dict:
    ...
    Args:
        grade: Tea grade
        density_check: Result from check_density function
    ...
    if density_check["within_tolerance"]:
        # Premium pricing for good density
        ...
        # Reduced pricing for poor density
...
def generate_recommendations(grade: str, density_check: dict) -> str:
    ...
    if density_check["within_tolerance"]:
        if density_check["status"] == "Excellent":
            ...
            return f"Good quality {grade}. Density is within acceptable range. Suitable for standard markets."
        else:
            return f"Acceptable quality {grade}. Minor density variation detected."
    else:
        if "Low Density" in density_check["status"]:
            return f"⚠️ Quality concern: Density is {density_check['deviation_percentage']}% below standard. May indicate moisture content issues or improper processing."
        else:
            return f"⚠️ Quality concern: Density is {density_check['deviation_percentage']}% above standard. May indicate compression or contamination."
...
def calculate_quality_score(confidence: float, density_check: dict) -> float:
    ...
    Combines:
    - Model confidence (50% weight)
    - Density compliance (50% weight)
    """
    ...
    if density_check["within_tolerance"]:
        ...
        deviation = min(density_check["deviation_percentage"], 50)  # Cap at 50%
...
async def predict_tea_grade(
    ...
    weight: float = Form(..., description="Weight in grams (for 1 liter volume)", gt=0),
    ...
    - **image**: Upload an image of the tea
    - **weight**: Weight of tea in grams (for the specified volume)
    - **volume**: Volume of container in liters (default: 1.0L)
    """
    if model is None:
    ...
    try:
        ...
        predicted_grade = CLASS_NAMES[class_index]
        ...
        # Get model accuracy
        if MODEL_METRICS:
            model_accuracy = MODEL_METRICS.get("overall_accuracy", 0.0)
        else:
            model_accuracy = 0.0  # Unknown if metrics not available
